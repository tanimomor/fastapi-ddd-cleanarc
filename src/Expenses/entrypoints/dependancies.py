...
from src.Expenses.entrypoints.schemas import CreateExpenseCategorySchema
from src.Expenses.service_layer.service import ExpenseCategoryService
from src.Expenses.service_layer.units_of_work import SQLAlchemyExpenseCategoryUnitOfWork
...
async def get_expense_category_by_id(category_id: int) -> ExpenseCategoryModel:
    service: ExpenseCategoryService = ExpenseCategoryService(uow=SQLAlchemyExpenseCategoryUnitOfWork())
    category: ExpenseCategoryModel = await service.get_category_by_id(id=category_id)
    if not category:
